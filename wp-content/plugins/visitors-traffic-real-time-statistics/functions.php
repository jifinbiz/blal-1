...
function ahcfree_get_recent_visitors($all, $cnt = true, $start = '', $limit = '', $year = '', $month = '', $ip = '')
{
...
$r_to_dt = date('m-t-Y', strtotime($r_from_dt));
    ...
    if ($r_from_dt != '' && $r_to_dt != '') {
        $fromdt = ahcfree_getFormattedDate($r_from_dt, 'yymmdd');
        $r_to_dt = ahcfree_getFormattedDate($r_to_dt, 'yymmdd');
...
function ahcfree_include_scripts()
{
    ...
    wp_register_style('ahc_lang_css', plugins_url('/css/vtrts_css_stylesheet.css?v=1.1', AHCFREE_PLUGIN_MAIN_FILE), '', '1.4');
    ...
    wp_register_script('ahc_main_js', plugins_url('/js/ahcfree_js_scripts.js?v=1.4', AHCFREE_PLUGIN_MAIN_FILE), '', '2.5');
    wp_enqueue_script('ahc_main_js');

    wp_localize_script('ahc_main_js', 'ahc_ajax', array('ajax_url' => admin_url('admin-ajax.php'),'nonce' => wp_create_nonce('ajax-nonce')));
...
function ahcfree_recent_visitor_by_ip_callback()
{
    ...
    if ( ! wp_verify_nonce( $_REQUEST['nonce'], 'ajax-nonce' ) ) {
        ...
        $year = ahc_free_sanitize_text_or_array_field($_REQUEST['year']);
        $month = ahc_free_sanitize_text_or_array_field($_REQUEST['month']);
        $start = ahc_free_sanitize_text_or_array_field($_REQUEST['start']);
        $length = ahc_free_sanitize_text_or_array_field($_REQUEST['length']);

        $cnt = ahcfree_get_recent_visitors("", true, "", "", $year, $month, $ip);
        $recentVisitors = ahcfree_get_recent_visitors("", false, $start, $length, $year, $month, $ip);
